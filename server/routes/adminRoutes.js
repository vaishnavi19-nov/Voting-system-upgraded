...
router.patch('/election/status', (req, res) => {
  ...
  const election = queryGet(`SELECT id, status FROM elections ORDER BY created_at DESC LIMIT 1`);
  if (!election) return res.status(404).json({ error: 'No election found.' });

  if (election.status === 'CLOSED' && status !== 'CLOSED') {
    return res.status(403).json({ error: 'Security Lock: A closed election cannot be reopened. You must reset votes to start a new election.' });
  }
...
router.post('/reset-votes', (req, res) => {
  ...
  const election = queryGet(`SELECT id FROM elections ORDER BY created_at DESC LIMIT 1`);
  if (election) {
    queryRun(`UPDATE elections SET status = 'NOT_STARTED' WHERE id = ?`, [election.id]);
  }
  
  res.json({ success: true, message: 'All votes have been reset and election state is now NOT STARTED.' });
});
...
// 9. Export Detailed CSV Results with Timestamps
router.get('/detailed-export', (req, res) => {
  const election = queryGet(`SELECT id, title FROM elections ORDER BY created_at DESC LIMIT 1`);
  if (!election) return res.status(404).json({ error: 'No election found.' });

  const votes = queryAll(
    `SELECT v.id, v.created_at, p.display_name as position, c.name as candidate, c.party_affiliation as party
     FROM votes v
  ...
  let csvContent = `Vote ID,Timestamp (IST),Position,Candidate,Party\n`;
  for (const vote of votes) {
    // SQLite CURRENT_TIMESTAMP is in UTC (e.g. "2026-08-17 07:11:31"). Ensure it parses as UTC.
    const dateStr = vote.created_at.includes('Z') ? vote.created_at : vote.created_at.replace(' ', 'T') + 'Z';
    ...
    csvContent += `"${vote.id}","${istTime}","${vote.position}","${vote.candidate}","${vote.party}"\n`;
  }

  res.setHeader('Content-Type', 'text/csv');
  res.setHeader('Content-Disposition', `attachment; filename="detailed_election_logs_${Date.now()}.csv"`);
  res.send(csvContent);
});

// 10. Terminate Active Voting Session
router.post('/terminate-active-session', (req, res) => {
  const election = queryGet(`SELECT id FROM elections ORDER BY created_at DESC LIMIT 1`);
  if (!election) return res.status(404).json({ error: 'No election found.' });
  
  queryRun(`UPDATE voting_sessions SET status = 'COMPLETED' WHERE election_id = ? AND status = 'ACTIVE'`, [election.id]);
  res.json({ success: true, message: 'All active voting sessions have been terminated.' });
});
