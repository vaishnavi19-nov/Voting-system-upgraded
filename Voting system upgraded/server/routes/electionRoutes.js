...
function getActiveElection() {
  ...
  // Auto-close if end_time has elapsed and status is still ACTIVE
  if (currentStatus === 'ACTIVE' && now >= endTime) {
    currentStatus = 'CLOSED';
    queryRun(`UPDATE elections SET status = 'CLOSED' WHERE id = ?`, [election.id]);
    election.status = 'CLOSED';
  }

  return { ...election, currentStatus };
}
...
router.post('/vote', (req, res) => {
  ...
  // Check 1: Is election active & timing valid?
  const election = getActiveElection();
  ...
  if (now >= endTime) {
    queryRun(`UPDATE elections SET status = 'CLOSED' WHERE id = ?`, [election.id]);
    return res.status(403).json({ error: 'Voting deadline has passed. Election is now closed.' });
  }
